...
static int toggles_init() {
    if (!toggles_initialized) {
        ...
        toggle_timer = xTimerCreate(
            "Toggle timer", pdMS_TO_TICKS(20), pdTRUE, NULL, toggle_timer_callback
        );
...
int toggle_create(const uint8_t gpio_num, toggle_callback_fn callback) {
    ...
    my_gpio_pullup(toggle->gpio_num);
